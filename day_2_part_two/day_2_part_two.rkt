...
; Return the minimum rgb of all games in a set
(define (min-rgb id_and_rest)
    (foldl
        (lambda (game acc)
            (map max game acc)
        )
        (list 0 0 0)
        (map game->rgb (games id_and_rest))
    )
)

; Power definition as per the rules
(define (power rgb) 
    (foldl * 1 rgb)
)

;(define all-lines (sequence->list (in-lines infile)))

;(displayln (first all-lines))
;(displayln (power (min-rgb (id-and-rest (third all-lines)))))

(displayln (sequence-fold
    (lambda (acc line) (+ (power (min-rgb (id-and-rest line))) acc))
    0
    (in-lines infile)
))